-- ============================================================
-- 014_pending_first_ordering.sql
-- Run AFTER 013_usage_tracking.sql.
--
-- The dashboard appointments list should surface bookings that need
-- action (status = 'pending') at the top, so admins see what to approve
-- or reject first. Within that grouping the existing newest-first order
-- by scheduled_at is preserved.
--
-- Same signature as 007, so CREATE OR REPLACE is enough — only the
-- ORDER BY changes. Still SECURITY INVOKER (default) so RLS applies.
-- ============================================================
